...
def process_break_command(
    ...
    if len(parts) < 2:
        return False, "Please provide a command after /break. Examples: '/break continue', '/break param=value', '/break start_over'"
        ...
        if agent:
            if hasattr(agent, 'state'):
                ...
                if update_memory_func:
                    update_memory_func("system", "User instructed to proceed with default values.")
    ...
    # Unknown command
    return False, f"Unknown command: {command}. Available commands: continue, start_over, or param=value"

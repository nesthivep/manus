...
class BaseAgent(BaseModel, ABC):
    ...
    async def handle_stuck_state(self):
        ...
        # Get user input with clear visual distinction
        print("\n" + "-"*50)  # Add a visual separator
        user_input = input("\nYour response: ").strip()
        
        # Ensure we have actual input and not just whitespace
        while not user_input:
            print("The agent needs your input to continue. Please type something.")
        ...
        if user_input.startswith("/"):
            ...
            # Generic user input handling - treat all input as direct responses to the agent's question
            # Create an explicit prompt to ensure the agent recognizes this as the answer
            # to its previous question and doesn't repeat the same question
            self.next_step_prompt = f"The user has responded with: '{user_input}'. Use this information to continue the conversation without repeating your previous question."
            logger.info(f"User provided input: {user_input}")
            
            # Add a system message with explicit instructions not to repeat questions
            self.update_memory("system", "User has provided their response. Do not repeat previous questions.")
